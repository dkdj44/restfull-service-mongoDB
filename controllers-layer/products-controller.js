...
router.get("/prices/:price1/:price2", async (request, response) => {
    try {
        ...
        const products = await logic.getProductsByPricesAsync(price1, price2);
        response.json(products);
    }
    catch (err) {
        response.status(500).send(err.message);
...
// GET by min stock http://localhost:3001/api/products/min-stock/20
router.get("/min-stock/:minStock", async (request, response) => {
    try {
        const minStock = +request.params.minStock;
        const products = await logic.getProductsByMinStock(minStock);
        response.json(products);
    }
    catch (err) {
        response.status(500).send(err.message);
